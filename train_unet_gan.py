...
def read_imgs_keraspp(imgs_df):
    imgs_tensor = np.zeros((len(imgs_df)*3, 224, 224, 3))
    imgs_labels = np.zeros((len(imgs_df)*3, 224, 224, 3))
#    imgs_labels = np.zeros((len(imgs_df), 224, 224, 1))
    for index, (_, row) in enumerate(imgs_df.iterrows()):
        prev_image_path = row['image'].replace('SENTINEL', '0SENTINEL')
        next_image_path = row['image'].replace('SENTINEL', '2SENTINEL')
        ...
        imgs_tensor[index*3, :, :, :] = np.array(Image.open(prev_image_path).resize([224, 224]).convert('RGB'))
        imgs_tensor[index*3, :, :, :] = np.array(Image.open(prev_image_path).resize([224, 224]).convert('RGB'))
        
        imgs_labels[index*3 + 1, :, :, :] = np.array(Image.open(next_image_path).resize([224, 224]).convert('RGB'))
        imgs_labels[index*3 + 1, :, :, :] = np.array(Image.open(next_image_path).resize([224, 224]).convert('RGB'))
        imgs_tensor[index*3 + 2, :, :, :] = np.array(Image.open(row['image']).resize([224, 224]).convert('RGB'))
        imgs_labels[index*3 + 2, :, :, :] = np.array(Image.open(row['image']).resize([224, 224]).convert('RGB'))
#        imgs_labels[index, :, :, 0] = np.array(Image.open(row['image']).resize([224, 224]).convert('RGB'))[:, :, 0]
...
filepath="best-unet-gan-all.hdf5"
checkpoint = ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_best_only=True, mode='min')
csv_logger = CSVLogger('log_512_200_unet_all.csv', append=True, separator=';')
callbacks_list = [checkpoint, csv_logger]

model.fit_generator(batch_generator(train_df, batch_size), steps_per_epoch=round(len(train_df)/batch_size),
        epochs=70, validation_data=batch_generator(val_df, batch_size), validation_steps=round(len(val_df)/batch_size),callbacks=callbacks_list)
