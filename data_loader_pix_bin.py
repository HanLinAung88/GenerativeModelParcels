...
class DataLoader():
    ...
    def load_data(self, batch_size=1, is_testing=False):
        ...
        imgs_A = []
        imgs_B = []
        for index, row in batch_images.iterrows():
            #img_B, img_A = self.imread(img_path_tpl['image']), self.imread(img_path_tpl['mask'])
            img_B = np.array(Image.open(row['image']).resize([256, 256]).convert('RGB')) #self.imread(row['image'])
            img_A = np.array(Image.open(row['mask']).resize([256, 256]))/255.0   #self.imread(row['mask'])
...
#            scipy.misc.imsave('outfile_true_B_pix.jpg', imgs_B[0])
            yield imgs_B, imgs_A
