...
def read_csv(original, destination, csv_file='data/testFR_450mbbox.csv'):
  grid = dict()
...
# read the shapefile
def dump_shp_to_json(shape_file, grid):
  ...
  for sr in reader.iterShapeRecords():
    ...
    # write the GeoJSON file
      geojson = open("pyshp-all-2000-sentinel.json", "w")
      geojson.write(dumps({"type": "FeatureCollection",\
      "features": buffer}, indent=2) + "\n")
      geojson.close()


def dump_shp_to_json_random(shape_file, num_random=2000):
  reader = shapefile.Reader(shape_file)
  original = Proj(fiona.open(shape_file).crs)
  print(fiona.open(shape_file).crs)
  destination = Proj('epsg:4326')
  ...
  buffer = []
  index = 0
  rand_indices = np.random.choice(len(reader) - 100, num_random//100)
  print(rand_indices)
  for rand_index, i in enumerate(rand_indices):
    for increment in range(100):
      sr = reader.shapeRecord(i + increment)
      atr = dict(zip(field_names, sr.record))
      geom = sr.shape.__geo_interface__
      geom['coordinates'] = listit(geom['coordinates'])
      for index_coord in range(0, len(geom['coordinates'])):
        for counter in range(0,len(geom['coordinates'][index_coord])):
          ...
          geom['coordinates'][index_coord][counter] = [lat, long] #(long, lat)
      buffer.append(dict(type="Feature", geometry=geom, properties=atr))
    print("Finished batch of 100: ", rand_index)
      # write the GeoJSON file
  geojson = open("pyshp-2000-100batch.json", "w")
  geojson.write(dumps({"type": "FeatureCollection",\
  "features": buffer}, indent=2) + "\n")
  geojson.close()


csv_file = '/atlas/u/hanlaung/ParcelDelineation/data/sentinel/sentinel_locations_apr_jun_1000.csv'
shape_file = '/atlas/u/hanlaung/RPG_2-0__SHP_LAMB93_FR-2017_2017-01-01/RPG/1_DONNEES_LIVRAISON_2017/RPG_2-0_SHP_LAMB93_FR-2017/PARCELLES_GRAPHIQUES.shp'
original = Proj(fiona.open(shape_file).crs)
print(original)
destination = Proj('epsg:4326')
reader = shapefile.Reader(shape_file)
